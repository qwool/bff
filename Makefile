...
all:
	@echo Run \'make install\' to install fff or run \'make install-bff\' to install it as bff.
...
install-bff:
	...
	@mkdir -p $(DESTDIR)$(MANDIR)/man1
	@mkdir -p $(DESTDIR)$(DOCDIR)
	@cp -p fff $(DESTDIR)$(PREFIX)/bin/bff
	@cp -p fff.1 $(DESTDIR)$(MANDIR)/man1
	@cp -p README.md $(DESTDIR)$(DOCDIR)
...
uninstall:
	...
	@rm -rf $(DESTDIR)$(PREFIX)/bin/bff
	@rm -rf $(DESTDIR)$(MANDIR)/man1/bff.1
	@rm -rf $(DESTDIR)$(DOCDIR)
